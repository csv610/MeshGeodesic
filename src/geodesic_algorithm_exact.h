 ...
 * @brief Implementation of the Mitchell-Mount-Papadimitriou (MMP) algorithm for exact geodesics.
 * 
 * This algorithm computes mathematically exact shortest paths on triangle meshes.
 * Reference: Mitchell, J. S., Mount, D. M., & Papadimitriou, C. H. (1987). 
...
private:
	...
	std::size_t compute_propagated_parameters(double pseudo_x, 
											double pseudo_y, 
	...
	std::size_t intersect_intervals(interval_pointer zero, 
	...
	bool check_stop_conditions(std::size_t& index);
	...
	std::size_t m_queue_max_size;			//used for statistics
	std::size_t m_iterations;			//used for statistics
