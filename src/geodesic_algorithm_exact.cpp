...
GeodesicAlgorithmExact::GeodesicAlgorithmExact(geodesic::Mesh* mesh):
	...
	m_edge_interval_lists(mesh->edges().size())
{
	...
	for(std::size_t i=0; i<m_edge_interval_lists.size(); ++i)
	{
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		...
		std::size_t const check_period = 10;
		...
		for(std::size_t i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
		{
			...
			std::size_t num_propagated = compute_propagated_parameters(min_interval->pseudo_x(), 
...
std::size_t GeodesicAlgorithmExact::best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
												double& best_source_distance)
{
	double best_interval_position; 
	std::size_t best_source_index;
												...
												std::size_t num_candidates)
{
	...
	for(std::size_t i=0; i<num_candidates; ++i)				//for all new intervals
	{
		...
		{
			std::size_t const N = intersect_intervals(p, q);								//interset two intervals
			...
			for(std::size_t j=1; j<N; ++j)				//no memory is needed for the first one
			{
			...
			for(std::size_t j=0; j<N; ++j)								//find "min" and add the intervals to the queue
			{
				if(j==N-1 && map[j]==NEW)
				{
					previous = i_new[j];
...
std::size_t GeodesicAlgorithmExact::compute_propagated_parameters(double pseudo_x, 
																		double pseudo_y, 
...
std::size_t GeodesicAlgorithmExact::intersect_intervals(interval_pointer zero, 
															   IntervalWithStop* one)			//intersecting two intervals with up to three intervals in the end
{
	...
	std::size_t Ninter=0;										//number of the points of the intersection
	...
	good_start[0] = left;
	std::size_t Ngood_start=1;										//number of the points of the intersection	

	for(std::size_t i=0; i<Ninter; ++i)							//for all points of intersection
	{
		double x = inter[i];
	...
	MapType mid_map[3];
	for(std::size_t i=0; i<Ngood_start-1; ++i)
	{
	...
	for(std::size_t i=0;i<Ngood_start-1;++i)							//for all intervals
	{
...
void GeodesicAlgorithmExact::initialize_propagation_data()
{
	...
	for(std::size_t i=0; i<m_sources.size(); ++i)		//for all edges adjacent to the starting vertex			
	{
		...
		for(std::size_t j=0; j<edges_visible_from_source.size(); ++j)
		{
...
bool GeodesicAlgorithmExact::check_stop_conditions(std::size_t& index)
{
